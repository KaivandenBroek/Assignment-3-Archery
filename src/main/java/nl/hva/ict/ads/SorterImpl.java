...
public class SorterImpl<E> implements Sorter<E> {
     ...
     * for deciding relative ordening of two items Items are sorted 'in place'
     * without use of an auxiliary list or array
     * 
     * @param items
     * @param comparator
    ...
    public List<E> quickSort(List<E> items, Comparator<E> comparator) {
        // sort the complete list of items from position 0 till size-1, encluding
        // position size
     ...
     * criteria of comparator and organizes and sorts this lead collection into the
     * first numTops positions of the list with use of (zero-based) heapSwim and
     * heapSink operations. The remaining items are kept in the tail of the list, in
     * arbitrary order. Items are sorted 'in place' without use of an auxiliary list
     * or array or other positions in items
     * 
     ...
     *         comparator all other items >= any item in the lead collection
     */
     ...
     * the comparator all items[0..heapSize-2] are assumed to satisfy the heap
     * condition The zero-bases heap condition says: all items[i] <= items[2*i+1]
     * and items[i] <= items[2*i+2], if any or equivalently: all items[i] >=
     * items[(i-1)/2]
     * 
     * @param items
     * @param heapSize
     ...
     * the comparator all items[1..heapSize-1] are assumed to satisfy the heap
     * condition The zero-bases heap condition says: all items[i] <= items[2*i+1]
     * and items[i] <= items[2*i+2], if any or equivalently: all items[i] >=
     * items[(i-1)/2]
     * 
